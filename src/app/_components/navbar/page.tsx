    ...
    <div id="main-nav" className={`fixed top-0 left-0 right-0 z-50 w-full max-w-7xl mx-auto mt-8 my-5 px-4 ${isShrunk ? 'nav-shrunk' : ''}`}>
      ...
      >
        <div className="text-xl flex gap-5 font-black text-green-500 tracking-tighter">
          <Image src={img} alt="FreshCart Logo" width={130} height={80} priority />
        </div>
        ...
        <div className="flex items-center gap-4">
                ...
                <button className="relative p-2 text-gray-700 hover:text-green-600" aria-label="Favorites">
                  {dataOfWishList?.count > 0 && (
                    <span className="absolute top-0 right-0 bg-green-500 text-white text-[10px] rounded-full h-4 w-4 flex items-center justify-center">
                      {dataOfWishList?.count}
                    </span>
                  )}
                  <svg xmlns="http://www.w3.org/2000/svg" className={`h-6 w-6 ${dataOfWishList?.count > 0 ? 'text-red-500' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {status === 'authenticated' ? (
            <button onClick={handleLogOut}>
              <span className="sm:flex items-center gap-2 bg-[rgb(49,243,49)] text-white px-5 py-2 rounded-full text-sm font-bold hover:bg-green-700 shadow-lg shadow-green-200 transition">
                Log Out
              </span>
            </button>
          ) : (
            <Link href="/login" className="sm:flex items-center gap-2 bg-[rgb(49,243,49)] text-white px-5 py-2 rounded-full text-sm font-bold hover:bg-green-700 shadow-lg shadow-green-200 transition">
              Sign In
            </Link>
          )}
        </div>
